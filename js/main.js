  ...
  function _subscribeForm() {
    ...
    $form.on('submit', function(e) {
     ...
     // if (!_formValidation(subscribeEmailVal)) {
      //  $subscribeNotice.stop(true).hide().html('<i class="fa fa-close error"></i> email inválido').fadeIn();
     ...
     // else {
        $.ajax({
          ...
          success: function() {
            ...
            $subscribeNotice.stop(true).hide().html('<i class="fa fa-check valid"></i> Obrigado! Entraremos em contato em breve.').fadeIn();
            $btnSubscribe.prop('disabled', false);
            $form[0].reset();
            $subscribeEmail.blur();
...
=================================================*/
  ...
  function _videoHeader() {
    ...
    var $body = $('body');
        ...
        if (isMobile) {
      $('#service').backstretch('img/bg/video-mobile.jpg'); // set a background image for mobile
    }
    else {
      ...
      $('#service').backstretch('img/bg/video-desktop.jpg'); // maybe need to change to css background style, please try resize the browser, i have tried it, work fine, but not sure other browser, will test tomorrow

      $bgVideo.attr('data-property', "{videoURL: __youtubeUrl, showControls: false, autoPlay: true, loop: true, mute: __videoMute, startAt: __videoStartTime, stopAt: __videoEndTime, quality: 'default', containment: '#service'}");
		$bgVideo.YTPlayer();

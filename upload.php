<?php
    ...
    {  
        $nome_completo = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(1, $row)->getValue()); 
        if ($nome_completo <> null ){
            ...
            $date = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(4, $row)->getValue());  
            $name_responsable = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(5, $row)->getValue());  
            $phone = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(6, $row)->getValue());  
            $email = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(7, $row)->getValue());
            $nickname = mysqli_real_escape_string($mysqli, $jogadores->getCellByColumnAndRow(8, $row)->getValue());
            ...
            players_name = 
            if ($id > $max_jogadores){
                $sql = "INSERT INTO players(whole_name, players_team_id, shirt, birthdate, name_responsible, phone, email, nickname) VALUES (UC_Words('".$nome_completo."'), (select id_teams from teams where teams_name  = '".$team_name."' and cup_id =".$copa."), '".$shirt."', '".$date."', '".$name_responsable."', '".$phone."', '".$email."', UC_Words('".$nickname."'), UC_Words(IF('".$nickname."' = "", CONCAT_WS(" ", substring_index('".$nome_completo."', " ", 1), substring_index('".$nome_completo."', " ", -1)), '".$nickname."')))";  
                mysqli_query($mysqli, $sql);  
            }else{ 
